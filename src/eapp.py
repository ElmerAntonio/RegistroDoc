...
class NotasFrame(ctk.CTkFrame):
    ...
    def crear_panel_derecho(self):
        ...
        ctk.CTkLabel(frame_der, text="Gestión de Notas", font=("Segoe UI", 18, "bold"), text_color="#3B82F6").pack(pady=(20, 10))

        ctk.CTkLabel(frame_der, text="Materia a calificar:", font=("Segoe UI", 13)).pack(anchor="w", padx=20)
        self.combo_materia = ctk.CTkOptionMenu(frame_der, values=["Cargando..."], command=self.cargar_descripciones)
        ...
        self.combo_tipo = ctk.CTkOptionMenu(tab_nueva, values=["Diaria / Parcial", "Apreciación", "Examen"], command=self.al_cambiar_tipo_nota)
        self.combo_tipo.pack(fill="x", padx=10, pady=10)

        ctk.CTkLabel(tab_nueva, text="Fecha (MM-DD):", font=("Segoe UI", 12)).pack(anchor="w", padx=10, pady=(5,0))
        ...
        self.btn_guardar_nueva = ctk.CTkButton(tab_nueva, text="💾 GUARDAR NUEVA", fg_color="#10B981", hover_color="#059669", 
                      font=("Segoe UI", 14, "bold"), height=40, command=self.guardar_notas)
        self.btn_guardar_nueva.pack(pady=20, padx=10, fill="x")

        # ====== TAB: MODIFICAR NOTA ======
        self.combo_trimestre_mod = ctk.CTkOptionMenu(tab_mod, values=["Trimestre 1", "Trimestre 2", "Trimestre 3"], command=self.cargar_descripciones)
        self.combo_trimestre_mod.pack(fill="x", padx=10, pady=5)

        self.combo_tipo_mod = ctk.CTkOptionMenu(tab_mod, values=["Diaria / Parcial", "Apreciación", "Examen"], command=self.cargar_descripciones)
        self.combo_tipo_mod.pack(fill="x", padx=10, pady=5)

        ctk.CTkLabel(tab_mod, text="Seleccione la nota a editar:", font=("Segoe UI", 12, "bold")).pack(anchor="w", padx=10, pady=(15,0))
        self.combo_desc_mod = ctk.CTkOptionMenu(tab_mod, values=["Seleccione parámetros arriba..."])
    ...
    def al_cambiar_tipo_nota(self, valor):
        ...
        if valor == "Examen":
            ...
            self.entry_desc.configure(placeholder_text="Ej. Charla, Taller, etc.")
    ...
    def cargar_descripciones(self, *args):
        ...
        descripciones = self.engine.obtener_descripciones_notas(grado, materia, trimestre, tipo)
        if descripciones:
    ...
    def guardar_notas(self):
        ...
        if notas_guardar is None or not notas_guardar:
            if notas_guardar == {}: messagebox.showinfo("Aviso", "No hay notas para guardar.")
            return
        ...
        def tarea_fondo():
            exito, msj = self.engine.guardar_columna_notas(grado, materia, trimestre, tipo, fecha, desc, notas_guardar)
            # Volvemos a la pantalla principal para devolver el botón a la normalidad
    ...
    def finalizar_guardado(self, exito, msj):
        self.btn_guardar_nueva.configure(text="💾 GUARDAR NUEVA", state="normal")
        if exito:
            self.cargar_descripciones()
        else:
    ...
    def buscar_modificar(self):
        ...
        if desc == "Sin notas registradas" or desc == "Seleccione parámetros arriba...":
            messagebox.showwarning("Atención", "No hay una nota válida seleccionada.")
            return
            
        resultado = self.engine.buscar_notas_por_descripcion_exacta(grado, materia, trimestre, tipo, desc)
        
        if not resultado:
            messagebox.showerror("Error", "No se encontraron las notas en el archivo.")
            return
    ...
    def actualizar_notas(self):
        ...
        def tarea_fondo_act():
            exito = self.engine.actualizar_notas_existentes(grado, materia, self.col_a_modificar, notas_guardar)
    ...
    def finalizar_actualizacion(self, exito):
        self.btn_actualizar.configure(text="🔄 ACTUALIZAR EXCEL", state="normal")
        self.col_a_modificar = None
        if not exito:
            messagebox.showerror("Error", "No se pudieron actualizar las notas.")